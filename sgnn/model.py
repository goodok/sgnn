...
class SparseEncoderLayer(nn.Module):
    ...
    def forward(self, x):
        if not self.input_sparsetensor:
            x = self.p0(x)
        # print('x', x.spatial_size, x.metadata.getSpatialLocations(x.spatial_size).shape,
        #       torch.max(x.metadata.getSpatialLocations(x.spatial_size)[:,:-1]).item(), x.features.shape)

        x = self.p1(x)
        # print('x(p1)', x.spatial_size, x.metadata.getSpatialLocations(x.spatial_size).shape,
        #       torch.max(x.metadata.getSpatialLocations(x.spatial_size)[:,:-1]).item(), x.features.shape)

        x = self.p2(x)
        ft2 = x
        # print('x(p2)', x.spatial_size, x.metadata.getSpatialLocations(x.spatial_size).shape,
        #       torch.max(x.metadata.getSpatialLocations(x.spatial_size)[:,:-1]).item(), x.features.shape)

        x = self.p3(x)
        # print('x(p3)', x.spatial_size, x.metadata.getSpatialLocations(x.spatial_size).shape,
        #       torch.max(x.metadata.getSpatialLocations(x.spatial_size)[:,:-1]).item(), x.features.shape)
...
class TSDFEncoder(nn.Module):
    def __init__(self, nf_in, nf_per_level, nf_out, use_skip_sparse, use_skip_dense, input_volume_size):
        ...
        params_encodedense = count_num_model_params(self.encode_dense0) + count_num_model_params(self.encode_dense0) + \
            count_num_model_params(self.encode_dense1) + count_num_model_params(self.bottleneck_dense2)
        params_decodedense = count_num_model_params(self.decode_dense3) + count_num_model_params(self.decode_dense4) + \
            count_num_model_params(self.final) + count_num_model_params(self.occpred)
...
class GenModel(nn.Module):
    def __init__(self, encoder_dim, input_dim, input_nf, nf_coarse, nf, num_hierarchy_levels, pass_occ, pass_feats,
                 use_skip_sparse, use_skip_dense, truncation=3):
        print("GenModel:")
        ...
        self.nf_per_level = [int(encoder_dim * (1 + float(k) / (num_hierarchy_levels - 2)))
                             for k in range(num_hierarchy_levels - 1)] if num_hierarchy_levels > 2 else [encoder_dim] * (num_hierarchy_levels - 1)
        self.use_skip_sparse = use_skip_sparse
    ...
    def dense_coarse_to_sparse(self, coarse_feats, coarse_occ, truncation):
        ...
        # sparse locations
        locs_unfilt = torch.nonzero(torch.ones([coarse_occ.shape[2], coarse_occ.shape[3], coarse_occ.shape[4]]))
        locs_unfilt = locs_unfilt.unsqueeze(0).repeat(coarse_occ.shape[0], 1, 1).view(-1, 3)
        batches = torch.arange(coarse_occ.shape[0]).to(locs_unfilt.device).unsqueeze(1)
        batches = batches.repeat(1, coarse_occ.shape[2] * coarse_occ.shape[3] * coarse_occ.shape[4]).view(-1, 1)
    ...
    def forward(self, x, loss_weights=None):
        ...
        if self.use_skip_sparse:
            for k in range(len(feats_sparse)):
                ...
                feats_sparse[k] = ([feats_sparse[k].metadata.getSpatialLocations(feats_sparse[k].spatial_size), scn.OutputLayer(3)(feats_sparse[k])],
...
if __name__ == '__main__':
    ...
    model = GenModel(encoder_dim=8, input_dim=(128, 128, 128), input_nf=4, nf_coarse=16, nf=16, num_hierarchy_levels=4, pass_occ=True, pass_feats=True,
                     use_skip_sparse=1, use_skip_dense=1)
