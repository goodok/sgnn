        ...
        if output_occs is not None:
            for h in range(num_hierarchy_levels):
                ...
                if target_for_occs is not None:
                    visualize_occ_as_points(target_for_occs[h][k, 0] == 1,
                                            0.5,
                                            os.path.join(output_path, name + 'target-' + str(h) + ext),
                                            transform,
                                            thresh_max=1.5)
                if output_occs is not None and output_occs[h][k] is not None:
                    visualize_sparse_locs_as_points(output_occs[h][k], os.path.join(output_path, name + 'pred-' + str(h) + ext),
                                                    transform)
